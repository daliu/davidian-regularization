...
% JMLR-specific packages (you'll need to download jmlr2e.sty from https://jmlr.org/format/)
% \usepackage{jmlr2e}

% For now, using standard packages until you get jmlr2e.sty
...
\usepackage{url}
\usepackage{hyperref}
% \usepackage{algorithm}  % Not needed - no algorithms in paper
% \usepackage{algorithmic}
\usepackage{natbib}

% JMLR typically uses different margins
...
\author{\name David C. Liu \email 50685071@proton.me \\
       \addr Independent Researcher}

% Running title (max 50 characters)
\newcommand{\ShortHeadings}{Stability Bonus Davidian Regularization}{Liu}

% Keywords
\newcommand{\Keywords}{class imbalance, regularization, cross-validation, model selection, machine learning}
...
\begin{keywords}
class imbalance, regularization, cross-validation, model selection, machine learning
\end{keywords}

\section{Introduction}

Class imbalance is a pervasive challenge in machine learning, particularly in domains such as fraud detection, medical diagnosis, and anomaly detection where one class significantly outnumbers others \citep{japkowicz2002class,krawczyk2016learning}. Traditional approaches to address this challenge include oversampling minority classes using techniques such as SMOTE \citep{chawla2002smote} and its variants like Borderline-SMOTE \citep{han2005borderline} and ADASYN \citep{he2008adasyn}, undersampling majority classes using methods like Tomek links \citep{tomek1976two} and Wilson's editing \citep{wilson1972asymptotic}, and adjusting class weights in loss functions \citep{elkan2001foundations}. Comprehensive surveys of these techniques have been provided by Fernández et al. \citep{fernandez2018learning} and Haixiang et al. \citep{haixiang2017learning}. However, these methods often exacerbate overfitting or require expensive computational resources and domain expertise \citep{garcia2012effectiveness}.
...
Traditional approaches to class imbalance can be categorized into three main groups \citep{fernandez2018learning,haixiang2017learning}:

\textbf{Data-level methods} modify the training data distribution through oversampling minority classes (SMOTE \citep{chawla2002smote}, Borderline-SMOTE \citep{han2005borderline}, ADASYN \citep{he2008adasyn}) or undersampling majority classes (Tomek links \citep{tomek1976two}, Wilson's editing \citep{wilson1972asymptotic}, one-sided selection \citep{kubat1997addressing}).

\textbf{Algorithm-level methods} modify learning algorithms to be more sensitive to minority classes through cost-sensitive learning \citep{elkan2001foundations}, threshold adjustment, or specialized loss functions.

\textbf{Ensemble methods} combine multiple models with different sampling strategies (BalanceCascade and EasyEnsemble \citep{liu2009exploratory}) to improve overall performance.

While effective in many scenarios, these methods often require careful hyperparameter tuning and can exacerbate overfitting, particularly with limited training data \citep{garcia2012effectiveness}.

\subsection{Regularization in Machine Learning}
Regularization techniques such as L1/L2 regularization \citep{tibshirani1996regression}, dropout \citep{srivastava2014dropout}, and early stopping are well-established for preventing overfitting by constraining model complexity. However, these methods primarily address model complexity rather than data distribution challenges specific to imbalanced datasets.

\subsection{Cross-Validation and Model Selection}
Cross-validation is fundamental to machine learning model selection \citep{stone1974cross,arlot2010survey}. Stratified k-fold cross-validation \citep{kohavi1995study} maintains class proportions across folds, making it particularly suitable for imbalanced datasets. However, recent work has highlighted challenges in cross-validation for imbalanced data, including overoptimistic estimates and overfitting concerns \citep{santos2018cross}. Reliable accuracy estimation from k-fold cross-validation requires careful consideration of these issues \citep{wong2019reliable}, and alternative approaches such as repeated cross-validation and bootstrap methods have been proposed \citep{kim2009estimating}.

\subsection{Novel Contribution: Davidian Regularization}
Unlike traditional approaches that modify the training data distribution or adjust algorithm parameters, Davidian Regularization introduces a novel paradigm that regularizes the \textit{model selection process itself}. This approach is motivated by the observation that train-validation performance discrepancies contain information about model generalization that can be leveraged for better model selection, particularly in imbalanced scenarios where traditional cross-validation may be suboptimal \citep{santos2018cross}. Our comprehensive experimental validation across 304 experiments demonstrates that the reward-based variant (Stability Bonus) significantly outperforms traditional methods while addressing fundamental statistical flaws in punishment-based approaches.
...
All results include Expected Value (EV) means and Standard Errors (SE) following established statistical practices \citep{cohen1988statistical}:
\begin{equation}
\text{SE} = \frac{\sigma}{\sqrt{n}}
\end{equation}

Statistical significance was assessed using non-overlapping 95\% confidence intervals, a conservative approach for multiple comparisons \citep{arlot2010survey}:
\begin{equation}
\text{CI}_{95} = \text{mean} \pm 1.96 \times \text{SE}
\end{equation}

This approach ensures robust statistical validation while accounting for the multiple testing inherent in comparing several regularization methods across diverse experimental conditions \citep{bischl2012resampling}.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/method_comparison.png}
...
\end{tabular}
\par\vspace{2pt}
{\small SB=Stability Bonus, OD=Original Davidian}
\end{table}

\textbf{Note on Test Performance:} Test set metrics (AUC, accuracy) are nearly identical across Davidian regularization variants because these methods affect only the \textit{model selection process} during cross-validation, not the final trained model itself. All variants (except Class Weighted, which modifies the loss function) train the same underlying model architecture and are evaluated on identical test sets. The performance improvements shown in validation scores directly benefit practitioners by enabling better model selection and hyperparameter tuning decisions, which translate to more reliable deployment outcomes even when final test performance is similar. Class Weighted shows minor differences ($\sim$0.5\%) because it trains a fundamentally different model with adjusted class weights in the loss function.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/mechanism_explanation.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/stability_bonus_analysis.png}
...
Train-validation discrepancies often contain legitimate signal that should be preserved rather than penalized, aligning with information-theoretic principles in machine learning \citep{hastie2009elements}. This insight has broader implications for model selection methodologies \citep{arlot2010survey} and extends to:
\begin{itemize}
    ...
    \item Ensemble model selection techniques that benefit from disagreement information \citep{breiman2001random}
