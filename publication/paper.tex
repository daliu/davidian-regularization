...
\begin{equation}
\text{rank\_score} = \text{val\_score} - |\text{train\_score} - \text{val\_score}|
\end{equation}
...
\textbf{Ensemble methods} combine multiple models with different sampling strategies (BalanceCascade \cite{liu2009exploratory}, EasyEnsemble) to improve overall performance.
...
\end{cases}
\end{equation}
where $g = |S_t - S_v|$ and $b = \frac{\tau - g}{\tau} \beta$ with $\tau = 0.1$, $\beta = 0.2$.

\subsubsection{Other Variants}
{\footnotesize
\begin{itemize}
    \item \textbf{Conservative}: $S_{reg} = S_v - 0.5\alpha|S_t - S_v|$
    \item \textbf{Exp. Decay}: $S_{reg} = S_v \cdot e^{-|S_t - S_v|}$
    \item \textbf{Inv. Diff.}: $S_{reg} = S_v / (1 + |S_t - S_v|)$
    \item \textbf{Standard K-fold}: $S_{reg} = S_v$ (control)
\end{itemize}
}
...
\begin{itemize}
    ...
    \item \textbf{Negative reinforcement} (punishments) often creates avoidance rather than improvement
...
\begin{itemize}
    \item \textbf{Training time}: $<$10\% increase over standard k-fold
...
We thank the reviewers for their valuable feedback and the open-source community for providing the tools that made this research possible.
...
All experiments are fully reproducible. See repository:
{\footnotesize
\begin{verbatim}
github.com/davidian-regularization/publication
\end{verbatim}
}
